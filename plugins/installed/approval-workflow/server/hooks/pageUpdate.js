/**
 * Page Update Hook - Approval Workflow
 *
 * Creates an approval request when a page is updated
 * Migrated to Phase 1 hook system (page:update)
 */
module.exports = async function (data) {
  try {
    const { page, user, previousVersion } = data
    ...
    // Check if edits require approval
    if (!config.requireApprovalForEdit) {
    ...
    if (config.autoApproveAdmins && user.isAdmin) {
      this.logger.info(`Auto-approving page edit by admin: ${user.name}`)
      return data
    }
    ...
    if (pathSetting) {
      ...
      if (pathSetting.autoApprove) {
        this.logger.info(`Auto-approving edit for path: ${page.path}`)
        return data
      }
    }
    ...
    const contentLengthChange = page.content.length - (previousVersion?.content?.length || 0)
    const changeDescription = `Page updated: ${page.title} (${contentLengthChange > 0 ? '+' : ''}${contentLengthChange} characters)`
    ...
    const approval = await Approval.query().insert({
      ...
      isNew: false,
      changeDescription,
      previousContent: previousVersion?.content?.substring(0, 500) // Store first 500 chars for comparison
    })

    this.logger.info(`Created approval request #${approval.id} for UPDATED page ${page.id} by ${user.name}`)
    ...
    this.logger.error(`Error in page update hook: ${err.message}`)
